  ...
  return (
    <Box direction="vertical">
      <Layout justifyItems="center">
        <Cell span={4}>
          <Button prefixIcon={<Icons.FullScreen/>} onClick={() => openModal('full')}>Full</Button>
        </Cell>
        <Cell span={4}>
          <Button prefixIcon={<Icons.Desktop/>} onClick={() => openModal('desktop')}>Desktop</Button>
        </Cell>
        <Cell span={4}>
          <Button prefixIcon={<Icons.Mobile/>} onClick={() => openModal('mobile')}>Mobile</Button>
        </Cell>
      </Layout>
      <Modal screen={currentScreen} isOpen={isModalOpened} onRequestClose={closeModal}>
        {renderModalContent()}
      </Modal>
    </Box>
  );
    ...
    <Layout>
      ...
      <Cell>
        <FormField label="Valid until">
          <DatePicker value={new Date('April 1, 2022')} width="100%" />
        </FormField>
      </Cell>
    </Layout>
  );
